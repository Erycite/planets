...
ypos = [4,5,6,12,13,14]
#colos = ['b', 'r', 'm', 'orange']
#colos= ['#191439', '#241c50', '#2e2468','#382c81','#423399','#4b3ab2','#5a48c4','#6f5fcc']
colos = ['#a9425f','#9fb843','#49c2a0','#6b51ca', '# 326881',' # 7e3490','# 9f5635','# 4cae35' ]
ls = ['--', '-.', ':','-']
...
B = [[[4,10], [4,11], [4,12], [5,10],[5,11], [5,12], [6,11], [6,12]],
     [[7,10], [7,11], [7,12], [7.5,10],[7.5,11], [7.5,12], [8,11], [8,12]],
     [[5,8], [5,10], [5,12], [5.5,8],[5.5,10], [6,8], [6,10], [8,12]]]
for bands in B:
    #bands = [[4,8], [7,11]]
    plt.figure()
    ##############################################################################################
    ...
    for band_ind, band in enumerate(bands):
        iselect = [0,1,2,3]*20
        i = iselect[band_ind]
        j = np.repeat([0,1,2,3],4)[band_ind]
        ...
        plt.hlines(band, 0, time.max(), color=colos[i], linestyles= ls[j])
        for ph in phasic_epo:
            plt.hlines( (12+ (ypos[i]/10)), ph[0]*(1/sr), ph[1]*(1/sr), color=colos[i], linestyle=ls[j])
